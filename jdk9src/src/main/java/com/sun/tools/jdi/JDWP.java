...
class JDWP {
    ...
    static class ModuleReference {
         ...
         * <p>Since JDWP version 9.
         */
        static class CanRead {
            static final int COMMAND = 3;

            static CanRead process(VirtualMachineImpl vm, 
                                ModuleReferenceImpl module, 
                                ModuleReferenceImpl sourceModule)
                                    throws JDWPException {
                PacketStream ps = enqueueCommand(vm, module, sourceModule);
                return waitForReply(vm, ps);
            }
            ...
            static PacketStream enqueueCommand(VirtualMachineImpl vm, 
                ...
                PacketStream ps = new PacketStream(vm, COMMAND_SET, COMMAND);
                if ((vm.traceFlags & VirtualMachineImpl.TRACE_SENDS) != 0) {
                    vm.printTrace("Sending Command(id=" + ps.pkt.id + ") JDWP.ModuleReference.CanRead"+(ps.pkt.flags!=0?", FLAGS=" + ps.pkt.flags:""));
                }
                if ((ps.vm.traceFlags & VirtualMachineImpl.TRACE_SENDS) != 0) {
                    ps.vm.printTrace("Sending:                 module(ModuleReferenceImpl): " + (module==null?"NULL":"ref="+module.ref()));
                }
                ps.writeModuleRef(module.ref());
                if ((ps.vm.traceFlags & VirtualMachineImpl.TRACE_SENDS) != 0) {
                    ps.vm.printTrace("Sending:                 sourceModule(ModuleReferenceImpl): " + (sourceModule==null?"NULL":"ref="+sourceModule.ref()));
                }
                ps.writeModuleRef(sourceModule.ref());
                ps.send();
                return ps;
            }

            static CanRead waitForReply(VirtualMachineImpl vm, PacketStream ps)
                                    throws JDWPException {
                ps.waitForReply();
                return new CanRead(vm, ps);
            }


            /**
             * true if this module can read the source module; false otherwise.
             */
            final boolean canRead;

            private CanRead(VirtualMachineImpl vm, PacketStream ps) {
                if (vm.traceReceives) {
                    vm.printTrace("Receiving Command(id=" + ps.pkt.id + ") JDWP.ModuleReference.CanRead"+(ps.pkt.flags!=0?", FLAGS=" + ps.pkt.flags:"")+(ps.pkt.errorCode!=0?", ERROR CODE=" + ps.pkt.errorCode:""));
                }
                canRead = ps.readBoolean();
                if (vm.traceReceives) {
                    vm.printReceiveTrace(4, "canRead(boolean): " + canRead);
                }
            }
        }
    }
